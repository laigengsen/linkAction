<template>
  <section class="shortnet-input-box">
    <input type="text" class="input-box" placeholder="请输入需要转二维码的链接" v-model="longUrl"/>
    <span class="input-btn" @click="getLink">生成</span>
  </section>
</template>
...
export default {
  ...
  methods:{
    getLink(){
      ...
      this.$bus.$emit('createqrcode',this.config);
